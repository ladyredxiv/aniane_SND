...
function CountRetainers()
  if not Addons.GetAddon("RetainerList") then SomethingBroke("RetainerList", "CountRetainers()") end
...
function OpenRetainer(r)
  r = r - 1
  if not Addons.GetAddon("RetainerList") then SomethingBroke("RetainerList", "OpenRetainer("..r..")") end
  yield("/wait 0.3")
  --yield("/click RetainerList Retainers["..r.."].Select")
  SafeCallback("RetainerList", true, 2, r)
  yield("/wait 0.5")
  while Addons.GetAddon("SelectString")==false do
  ...
  if not Addons.GetAddon("SelectString") then SomethingBroke("SelectString", "OpenRetainer("..r..")") end
  yield("/wait 0.3")
  --yield("/click SelectString Entries[3].Select")
  SafeCallback("SelectString", true, 3)
  if not Addons.GetAddon("RetainerSellList") then SomethingBroke("RetainerSellList", "OpenRetainer("..r..")") end
end

function CloseRetainer()
  while not Addons.GetAddon("RetainerList") do
...
function ClickItem(item)
  CloseSales()
  while Addons.GetAddon("RetainerSell")==false do
    if Addons.GetAddon("ContextMenu") then
      SafeCallback("ContextMenu", true, 0, 0)
      yield("/wait 0.2")
    elseif Addons.GetAddon("RetainerSellList") then
...
function SearchResults()
  if Addons.GetAddon("ItemSearchResult")==false then
    yield("/wait 0.1")
    if Addons.GetAddon("ItemSearchResult")==false then
  ...
  if Addons.GetAddon("ItemHistory")==false then
    yield("/wait 0.1")
    if Addons.GetAddon("ItemHistory")==false then
...
function HistoryAverage()
  while Addons.GetAddon("ItemHistory")==false do
    SafeCallback("ItemSearchResult", true, 0)
    yield("/wait 0.3")
  end
...
function CloseSearch()
  while Addons.GetAddon("ItemSearchResult") or Addons.GetAddon("ItemHistory") do
    yield("/wait 0.1")
    if Addons.GetAddon("ItemSearchResult") then SafeCallback("ItemSearchResult", true, -1) end
    if Addons.GetAddon("ItemHistory") then SafeCallback("ItemHistory", true, -1) end
  end
end

function CloseSales()
  CloseSearch()
  while Addons.GetAddon("RetainerSell") do
    yield("/wait 0.1")
    if Addons.GetAddon("RetainerSell") then SafeCallback("RetainerSell", true, -1) end
  end
end

function SomethingBroke(what_should_be_visible, extra_info)
  for broken_rechecks=1, 20 do
    if Addons.GetAddon(what_should_be_visible) then
      still_broken = false
      break
    else
      yield("/wait 0.1")
...
function NextCharacter()
  current_character = Entity.Player.Name
  next_character = nil
...
function Relog(relog_character)
  ...
  while Svc.Condition[1] do
    yield("/wait 1.01")
  end
  while not Svc.Condition[1] do
    yield("/wait 1.02")
  end
  while Svc.Condition[45] or Svc.Condition[35] do
    yield("/wait 1.03")
  end
  yield("/wait 0.5")
  while Svc.Condition[35] do
    yield("/wait 1.04")
  end
  yield("/wait 2")
end
...
function EnterHouse()
  if Svc.ClientState.TerritoryType == 339 or Svc.ClientState.TerritoryType == 340 or Svc.ClientState.TerritoryType == 341 or Svc.ClientState.TerritoryType == 641 or Svc.ClientState.TerritoryType == 979 or Svc.ClientState.TerritoryType == 136 then
    ...
    if string.find(string.lower(Entity.Target.Name), "entrance") then
      while Svc.ClientState.TerritoryType == 339 or Svc.ClientState.TerritoryType == 340 or Svc.ClientState.TerritoryType == 341 or Svc.ClientState.TerritoryType == 641 or Svc.ClientState.TerritoryType == 979 or Svc.ClientState.TerritoryType == 136 do
...
function OpenBell()
  ...
  if Svc.Condition[50] then
    yield("/lockon off")
    while not Addons.GetAddon("RetainerList") do yield("/wait 0.100") end
    yield("/wait 0.4")
    return true
  else
    return false
...
function WaitARFinish(ar_time)
  ...
  while Addons.GetAddon("_TitleMenu")==false do
    yield("/wait 5.01")
  end
  while true do
    if Addons.GetAddon("_TitleMenu") and Addons.GetAddon("NowLoading")==false then
...
function SafeCallback(...)  -- Could be safer, but this is a good start, right?
  ...
  if IsAddonReady(addon) and Addons.GetAddon(addon) then
    yield(call_command)
  end
end
...
if Addons.GetAddon("RetainerList") then is_multimode = false end
...
::Startup::
Clear()
if not Svc.Condition[1] then
  echo("Not logged in?")
  yield("/wait 1")
  Relog(my_characters[1])
  goto Startup
elseif not Svc.Condition[50] then
  echo("Not at a summoning bell.")
  OpenBell()
  goto Startup
elseif Addons.GetAddon("RecommendList") then
  helper_mode = true
  while Addons.GetAddon("RecommendList") do
    SafeCallback("RecommendList", true, -1)
    yield("/wait 0.1")
  end
  echo("Starting in helper mode!")
  goto Helper
elseif Addons.GetAddon("RetainerList") then
  CountRetainers()
  goto NextRetainer
elseif Addons.GetAddon("RetainerSell") then
  echo("Starting in single item mode!")
  is_single_item_mode = true
  goto RepeatItem
elseif Addons.GetAddon("SelectString") then
...
while Addons.GetAddon("RetainerSell")==false do
  yield("/wait 0.5")
  if not Svc.Condition[50] or Addons.GetAddon("RecommendList") then
    goto EndOfScript
  end
end
...
::MultiMode::
if is_multimode then
  while Addons.GetAddon("RetainerList") do
...
if not Svc.Condition[50] and multimode_ending_command then
  yield("/wait 3")
